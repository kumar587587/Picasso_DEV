--------------------------------------------------------
--  DDL for Trigger XXCPC_CONTRACTPACKAGEDETA_BIUD
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE TRIGGER "WKSP_XXCPC"."XXCPC_CONTRACTPACKAGEDETA_BIUD" 
  BEFORE INSERT OR DELETE OR UPDATE ON "XXCPC_CONTRACTPACKAGEDETAILS"
  REFERENCING FOR EACH ROW
  DECLARE     
    c_trigger_name         CONSTANT VARCHAR2(50):= 'XXCPC_CONTRACTPACKAGEDETA_BIUD';  
    l_iud                  varchar2(1);
...
BEGIN       
   ...
   END IF;  
   --  
   insert into XXCPC_CONTRACTPACKAGEDETAILS_A( IUD,ID,CONTRACTID,PACKAGEID,STARTDATE,ENDDATE,UPDATEDFLAG,CREATED,CREATED_BY,UPDATED,UPDATED_BY,OPEN_PERIOD_I,OPEN_PERIOD_U,ID_OLD,CONTRACTID_OLD,PACKAGEID_OLD,STARTDATE_OLD,ENDDATE_OLD,UPDATEDFLAG_OLD,CREATED_OLD,CREATED_BY_OLD,UPDATED_OLD,UPDATED_BY_OLD,OPEN_PERIOD_I_OLD,OPEN_PERIOD_U_OLD,delete_period)   
   values ( l_iud, :new.ID, :new.CONTRACTID, :new.PACKAGEID, :new.STARTDATE, :new.ENDDATE, :new.UPDATEDFLAG, :new.CREATED, :new.CREATED_BY, :new.UPDATED, :new.UPDATED_BY, l_open_period_i, l_open_period_u , :old.ID, :old.CONTRACTID, :old.PACKAGEID, :old.STARTDATE, :old.ENDDATE, :old.UPDATEDFLAG, :old.CREATED, :old.CREATED_BY, :old.UPDATED, :old.UPDATED_BY, :old.OPEN_PERIOD_I, l_open_period_u, l_open_period_d);
   --
   xxcpc_gen_debug_pkg.debug_t( c_trigger_name, '(end)' );   
END xxcpc_contractpackagedeta_biud;

/
ALTER TRIGGER "WKSP_XXCPC"."XXCPC_CONTRACTPACKAGEDETA_BIUD" ENABLE;

--------------------------------------------------------
--  DDL for Package XXPM_SYNCHRONIZE_API
--------------------------------------------------------

  CREATE OR REPLACE EDITIONABLE PACKAGE "WKSP_XXPM"."XXPM_SYNCHRONIZE_API" 
IS        
  /****************************************************************       
   *       
   * PROGRAM NAME       
   *  XXPM_SYNCHRONIZE_API       
   *       
   * DESCRIPTION       
   ...
   **************************************************************/     
   gc_package_name   CONSTANT    VARCHAR(50) := 'XXPM_SYNCHRONIZE_API';
   ...
   g_arr_total  t_array;
   g_finished   NUMBER;
   g_processing_backlog_job_active  boolean := true;---Added by TechM team for MWT-796
   --    
   ---------------------------------------------------------------------------------------------
   ...
   -- Procedure to 
   ---------------------------------------------------------------------------------------------         
   FUNCTION create_select_tv_clause 
   ( p_pm_table       IN     VARCHAR2
   , p_key_fields      IN     VARCHAR2
   )
   RETURN VARCHAR2
   ...
   -- Function to 
   ---------------------------------------------------------------------------------------------  
   FUNCTION get_hash_on_table_via_api ( p_table_name  IN VARCHAR2 )
   RETURN RAW;
   --    
   ---------------------------------------------------------------------------------------------
   -- Function to 
   ---------------------------------------------------------------------------------------------  
   FUNCTION bubbel_sort ( p_input_array    IN  XXPM_array_varchar_type)
   RETURN XXPM_array_varchar_type;
   --
   PROCEDURE show_hash_diffs ( p_table_name IN VARCHAR2 );
   --
   ...
   -- Procedure that calls the main procedure for all records in XXPM_tables.   
   ---------------------------------------------------------------------------------------------         
   PROCEDURE synchronize( p_table_name IN VARCHAR2, p_pm_number_of_records IN NUMBER, p_context IN NUMBER , p_pm_hash IN VARCHAR2, p_tv_number_of_rows in NUMBER )          
   ;
   --    
   ---------------------------------------------------------------------------------------------
   ...
   , p_tv_number_of_records in NUMBER   
   );   
   ...
   , p_tv_all_fields      IN OUT VARCHAR2
   ...
   -- Function to 
   ---------------------------------------------------------------------------------------------  
   FUNCTION get_pm_content ( p_table_name IN VARCHAR2 )
   RETURN CLOB
   ;
   --    
   ---------------------------------------------------------------------------------------------
   -- Function to 
   ---------------------------------------------------------------------------------------------  
   FUNCTION get_hash (p_clob in clob )
   RETURN RAW
   ;
END XXPM_synchronize_api;

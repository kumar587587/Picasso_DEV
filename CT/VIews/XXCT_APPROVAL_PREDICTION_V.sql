--------------------------------------------------------
--  DDL for View XXCT_APPROVAL_PREDICTION_V
--------------------------------------------------------

  CREATE OR REPLACE FORCE EDITIONABLE VIEW "WKSP_XXCT"."XXCT_APPROVAL_PREDICTION_V" ("MANDATE_CYCLE", "DISPLAY_NAME", "ROLE_NAME", "APPROVAL_STATUS", "APPROVAL_DATE", "APPROVAL_COMMENTS", "ROLEID", "ACTIONTYPE", "DOSSIER_ID", "MAXAMOUNT", "MINAMOUNT", "TOTAL_AMOUNT") DEFAULT COLLATION "USING_NLS_COMP"  AS 
  SELECT nvl( v('P102_MANDATE_CYCLE'),d.mandate_cycle)   mandate_cycle
,      u.display_name
,      r.role_name
...
and p.partner_id            = nvl( v('P102_PARTNER_ID'),p.partner_id)
and apl.dossiertype         = nvl( v('P102_DOSSIER_TYPE_CODE'), apl.dossiertype)
and prdg.dossier_id         = nvl( v('P102_DOSSIER_ID'),prdg.dossier_id)
